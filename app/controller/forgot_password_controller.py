...
class ForgotPasswordController(QObject):
    ...
    def __init__(self, view: ForgotPassword, on_success: Callable, on_back: Callable, parent=None):
    ...
    # step 1 (email submission) → step 2 (OTP verification)
    ...
    def _on_send_email_result(self, result):
        try:
            ...
            if success:
                ...
                logger.debug("Password reset email sent successfully to: %s", self._email)
                return

            if error and ("rate limit" in error.lower() or "too many" in error.lower()):
                self.email_failed.emit("Too many reset requests. Please wait a bit and try again.")
            else:
    ...
    # step 2 (OTP verification) → Step 3 (password update)
    ...
    def _on_verify_otp_result(self, result):
        ...
        if success:
            self.view.show_otp_verified_success()
        else:
            if error == "token_expired":
                error_message = get_error_message("token_expired")
            elif error and ("expired" in error.lower() or "invalid" in error.lower() or "token" in error.lower()):
                error_message = get_error_message("token_expired")
            else:
                error_message = error or get_error_message("token_expired")
            self.view.show_otp_error(error_message)

    # step 3 (password update) → flow complete
    ...
    # silently resend the code when going back from password step with expired timer
    def _resend_code_silently(self, email: str):
        ...
        if not started:
            logger.debug("Forgot password operation already running, skipping silent resend")

    def _on_silent_resend_result(self, result):
        try:
            success, error = result
            if success:
                logger.debug("Silent resend of reset email succeeded for: %s", self._email)
            else:
