...
def _tick(user_id: str):
    """Send heartbeat with elapsed time since last tick."""
    global _last_tick
    now = datetime.now()
...
def start_heartbeat(user_id: str):
    """Start heartbeat timer to periodically update user activity."""
    if not user_id:
...
def flush_heartbeat():
    """Force-send accumulated time immediately at game boundaries."""
    if _current_user_id:
        _tick(_current_user_id)

def stop_heartbeat():
    """Stop heartbeat timer and send final accumulated time."""
    global _heartbeat_timer, _current_user_id, _last_tick
