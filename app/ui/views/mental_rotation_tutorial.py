...
# ── Shape rendering for tutorial examples ─────────────────────────────────
...
class MentalRotationTutorial(BaseTutorialWidget):
    ...
    def _get_intro_subtitle(self) -> str:
        return "Train your spatial reasoning and mental transformation skills"

    def _get_card_intro_html(self) -> str:
        return (
            f'Compare the <span style="color:{FONT_PRIMARY};">shapes</span> and decide if they are the '
            f'<span style="color:{FONT_PRIMARY};">same</span> or '
            f'<span style="color:{FONT_PRIMARY};">mirrored</span>.<br>'
            f'<span style="color:{FONT_PRIMARY};">Rotate</span> them mentally to find out.'
        )

    def _build_card_bullets(self, bullets_layout: QVBoxLayout) -> None:
        b1 = QLabel("• Two shapes appear \u2013 the original and a rotated or mirrored version.")
        ...
        lbl_same = QLabel("if the shapes are same,")
        ...
        lbl_diff = QLabel("if one is mirrored.")
        ...
        b4 = QLabel("• Complete <span style=\"color:#3EAC91;\">20 shapes</span> to finish the run.")
    ...
    def _get_practice_subtitle_html(self) -> str:
        return (
            "Let's learn how the "
            '<span style="color:#3EAC91;">Mental Rotation</span> '
            "works. Compare both shapes!"
        )
