"""
Centrálna logging infraštruktúra pre Synapso.

Poskytuje:
- SessionID pre koreláciu logov v rámci jedného behu
- Konzolový handler (farebný, development)
- Súborový handler (rotating, UTF-8, production)
- PID, thread name/ID v každom log riadku
- User ID injection po prihlásení
- Sanitizácia citlivých údajov
"""
...
def _get_log_dir() -> str:
    ...
    if getattr(sys, "frozen", False):
        # Frozen build (.exe) — use user-writable app data directory
        if sys.platform == "win32":
            base = os.environ.get("LOCALAPPDATA", os.path.expanduser("~"))
        else:
            base = os.path.expanduser("~/.local/share")
        return os.path.join(base, "Synapso", "logs")

    # Development — use project-local logs/
    return os.path.join(os.path.dirname(os.path.abspath(sys.argv[0] if sys.argv else __file__)), "logs")
...
# ── Root logger setup (called once at startup) ────────────────────
_initialized = False
...
def setup_logging() -> None:
    ...
    file_fmt = _ContextFormatter(
        fmt="%(asctime)s | %(levelname)-8s | %(session_id)s | PID:%(pid)s | %(threadName)s | %(user_id)s | %(name)s.%(funcName)s:%(lineno)d | %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S",
    )
    ...
    console_fmt = _ColorContextFormatter(
        fmt="%(asctime)s %(levelname)-8s | %(session_id)s | %(threadName)s | %(name)s | %(message)s",
        datefmt="%H:%M:%S",
    )
    ...
    # ── Optional level filter from env ──
